...
import { addRxPlugin, randomCouchString, newRxError, ensureNotFalsy, RxReplicationWriteToMasterRow, RxReplicationHandler, RxError, RxTypeError, RxCollection, rxStorageInstanceToReplicationHandler, getFromMapOrThrow } from 'rxdb';
import { RxDBLeaderElectionPlugin } from 'rxdb/plugins/leader-election';
import { replicateRxCollection } from 'rxdb/plugins/replication';
import { filter, map, BehaviorSubject, Subscription, Subject, firstValueFrom } from 'rxjs';
import {
...
export async function replicateWebRTC<RxDocType, PeerType>(
  ...
  pool.subs.push(
      pool.connectionHandler.error$.subscribe(err => {
        console.log('connectionHandler.error$', err)
        return pool.error$.next(err)
      }),
      pool.connectionHandler.disconnect$.subscribe(peer => {
      ...
      .subscribe(async (peer) => {
        debugger
          ...
          try {
            ...
            console.log('connect sub error', error)
              /**
